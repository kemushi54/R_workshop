...
* 相關 package 多 (IUCN redlist, trim)
...
<div class="text">
一個 package 會包含了許多功能類型相近的 function，R 語言的強大就是來自於擁有這些龐大的套件庫，由於 R 是開放原碼，除了 R 內建的 packages (Base R) 之外，也有許多由世界各地不同的開發者因為不同的需求所開發出來的套件，像是繪圖用的 `ggplot2`、資料處理用的 `dplyr`，或是文字處理的 `stringr`，。
</div>
...
<img src="image/table_compare.png"          style="width:100%; padding:0px">
...
### 我想用Ｒ做ＸＸＸ，但是沒有概念要怎麼做欸_(┐「ε:)_

關鍵字："想做的事(英文版) + R" 去問估狗大神

<p style="color:grey">估狗估狗估狗除了估狗還是估狗估狗估狗什麼都餵狗就對惹ก็ʕ。͡ᴥ。ʔ ก้</p>
