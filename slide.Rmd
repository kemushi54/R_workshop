...
<div>
1. 檔案格式：Excel, csv, txt(O); Word, PDF(X)
2. 不要合併儲存格
3. 欄位值格式一致

[資料整理入門（上）：認識你的資料格式](https://www.ragic.com/intl/zh-TW/blog/199/%E8%B3%87%E6%96%99%E6%95%B4%E7%90%86%E5%85%A5%E9%96%80%EF%BC%88%E4%B8%8A%EF%BC%89%EF%BC%9A%E8%AA%8D%E8%AD%98%E4%BD%A0%E7%9A%84%E8%B3%87%E6%96%99%E6%A0%BC%E5%BC%8F)
</div>
...
```{r echo = FALSE}
r_data <- 
  read.csv("D:/Project/Kemushi/R_workshop/r_data.csv")
r_data[1:10, ] %>% print()
```
</td>
</tr>
...
r_data %>% 
  setDT %>% 
  dcast(Site ~ Point, value.var = "Count", fun = sum) %>% 
  print()
```
...
```{r test}
w_data <- 
  read.csv("D:/Project/Kemushi/R_workshop/w_data.csv")
```
```{r echo = FALSE}
w_data[1:5, ] %>% 
  print()
